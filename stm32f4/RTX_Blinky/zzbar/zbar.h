...
static void* (*malloc_crane)(unsigned int num_bytes) = malloc;
static void *malloc_wrapper(unsigned int num_bytes){
void *p = malloc_crane(num_bytes);
printf("malloc\t0x%x\t0x%x\t%d\r\n",(int)p,num_bytes,num_bytes);
return p;
}
#define malloc(x) malloc_wrapper(x)

static void* (*realloc_crane)(void *mem_address, unsigned int newsize) = realloc;
static void *realloc_wrapper(void *mem_address, unsigned int newsize){
void *p = realloc_crane(mem_address,newsize);
printf("realloc\t0x%x\t0x%x\t%d\r\n",(int)p,newsize,newsize);
return p;
}
#define realloc(x,y) realloc_wrapper(x,y)

static void* (*calloc_crane)(unsigned n,unsigned size) = calloc;
static void *calloc_wrapper(unsigned n,unsigned size){
void *p = calloc_crane(n,size);
printf("calloc\t0x%x\t0x%x\t%d\r\n",(int)p,n*size,n*size);
return p;
}
